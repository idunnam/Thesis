...
"""
mask_ablation_ACCESS = ref_ACCESS.where(ref_ACCESS.SMB > 0)*mymask_test
#mask_ablation_HADGEM = ref_HADGEM.where(ref_HADGEM.SMB > 0)*mymask_test
mask_ablation_CSIRO = ref_CSIRO.where(ref_CSIRO.SMB > 0)*mymask_test
mask_ablation_IPSL = ref_IPSL.where(ref_IPSL.SMB > 0)*mymask_test
mask_ablation_MIROC5 = ref_MIROC5.where(ref_MIROC5.SMB > 0)*mymask_test
mask_ablation_NORESM = ref_NORESM.where(ref_NORESM.SMB > 0)*mymask_test

mask_ablation_CESM = ref_CESM.where(ref_CESM.SMB > 0)*mymask_test
mask_ablation_CNRM_CM6 = ref_CNRM_CM6.where(ref_CNRM_CM6.SMB > 0)*mymask_test
mask_ablation_CNRM_ESM2 = ref_CNRM_ESM2.where(ref_CNRM_ESM2.SMB > 0)*mymask_test
mask_ablation_MRI = ref_MRI.where(ref_MRI.SMB > 0)*mymask_test
mask_ablation_UKMO = ref_UKMO.where(ref_UKMO.SMB > 0)*mymask_test